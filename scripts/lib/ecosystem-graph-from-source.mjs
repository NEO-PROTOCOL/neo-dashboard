/**
 * Shared logic for sync-ecosystem-graph + validate-ecosystem-graph:
 * same source resolution and same node list (including cluster anchors) as the graph builder.
 */
import fs from "node:fs";
import path from "node:path";

// ── Source resolution (matches sync-ecosystem-graph.mjs) ──────────────────────

export async function loadEcosystemSourceArray(
  repoRoot,
  { silent = false } = {},
) {
  ...
  if (sourcePath) {
    try {
      ...
      if (sourceNodes?.length && !silent) {
        console.log(`[sync] source: local-file (${sourcePath})`);
      }
    } catch (e) {
      if (!silent) console.warn(`[sync] Failed to read ${sourcePath}:`, e.message);
    }
  }
  ...
  if (!sourceNodes?.length) {
    ...
    try {
      ...
      if (res.ok) {
        ...
        if (sourceNodes?.length && !silent) {
          console.log(`[sync] source: url (${url})`);
        }
      } else if (!silent) {
        console.warn(`[sync] URL returned HTTP ${res.status}`);
      }
    } catch (e) {
      if (!silent) console.warn(`[sync] URL fetch failed: ${e.message}`);
    }
  }

  return sourceNodes?.length ? sourceNodes : null;
}

// ── Transformation helpers (same as sync-ecosystem-graph.mjs) ───────────────
...
export function clusterAnchorId(group) {
  return `__anchor__:${String(group || "DApps & Tools")
    .toLowerCase()
    .replace(/[^a-z0-9]+/g, "-")}`;
}
...
function normalizeGroup(node) {
  ...
  if (org.includes("neo-growth-system") || mix.includes("growth system"))
    return "NEO-Growth-System";
  if (org.includes("flowpay") || mix.includes("flowpay-system") || mix.includes("flowpay"))
    return "FlowPay";
  if (
  ...
  if (org.includes("wod") || mix.includes("wodxpro") || mix.includes("wod"))
    return "WOD Game";
  if (
...
function computeNodeVal(node, group) {
  ...
  if (role.includes("orchestrator") || role.includes("event hub") || role.includes("sovereign"))
    return 12;
  if (id.includes("nexus") || id.includes("architect")) return 11;
  if (role.includes("protocol") || role.includes("contract")) return 9;
  if (
    group === "FlowPay" ||
    group === "Neo Smart Factory" ||
    group === "NEO-Growth-System" ||
    group === "WOD Game"
  )
    return 8;
...
/**
 * Dedupe + cluster anchors. `byGroup` is from the pre-anchor node list (used for link layout).
 */
export function buildNodesWithAnchors(sourceNodes) {
  const dedup = new Map();
...
export function expectedGraphNodeIdSet(sourceNodes) {
  const { nodes } = buildNodesWithAnchors(sourceNodes);
